...
public final class MainViewController implements Initializable {
  ...
  @FXML private Pane tuneWallsGroup;
  @FXML private Pane tuneReliefGroup;
  ...
  @FXML private ChoiceBox<ChoiceItem<MazeSolver>> solverChoiceBox;
  @FXML private ListView<String> solverResultMetadata;
  ...
  private final ObservableList<String> solverMetadataList = FXCollections.observableArrayList();

  @Override
  public void initialize(URL url, ResourceBundle resourceBundle) {
    canvasArea.heightProperty().addListener(this::onResize);
    canvasArea.widthProperty().addListener(this::onResize);
    tuneWallsGroup.managedProperty().bind(tuneWallsGroup.visibleProperty());
    tuneReliefGroup.managedProperty().bind(tuneReliefGroup.visibleProperty());
    solverResultMetadata.setItems(solverMetadataList);

    // Gestion dynamique de la taille de la ListView en fonction du nombre d'items, avec une taille max pour éviter une
    // ListView trop grande
    double cellHeight = 26;
    solverResultMetadata.setFixedCellSize(cellHeight);
    solverResultMetadata.prefHeightProperty().bind(Bindings.createDoubleBinding(
        () -> {
          int n = solverResultMetadata.getItems() == null ? 0 : solverResultMetadata.getItems().size();
          double vInsets = solverResultMetadata.getInsets().getTop() + solverResultMetadata.getInsets().getBottom();
          return n * solverResultMetadata.getFixedCellSize() + vInsets;
        },
        solverResultMetadata.getItems(), solverResultMetadata.insetsProperty()
    ));
    double maxVisibleRows = 6;
    solverResultMetadata.maxHeightProperty().bind(
        Bindings.createDoubleBinding(
            () -> maxVisibleRows * solverResultMetadata.getFixedCellSize()
                + solverResultMetadata.getInsets().getTop() + solverResultMetadata.getInsets().getBottom(),
            solverResultMetadata.insetsProperty()
        )
    );
    solverResultMetadata.visibleProperty().bind(Bindings.createBooleanBinding(
        () -> solverResultMetadata.getItems() != null && !solverResultMetadata.getItems().isEmpty(),
        solverResultMetadata.getItems()
    ));
  ...
  private void onGenerate() {
    ...
    tuneWallsGroup.setDisable(true);
    tuneReliefGroup.setDisable(true);
    ...
    if (!config.generatorRequiresWalls())
    ...
    Arrays.fill(weights, config.minWeight());

    painter.setMaze(maze)
        .setCellColorF(v -> config.generatorColor(maze, v));
    ...
    runOnWorker(() -> {
      try {
        ...
        builder.generate(maze, config.startPoint(maze));
      } catch (CanceledAnimationException _) {
      } finally {
      ...
      if (!canceled && autoTune.isSelected()) {
        if (config.allowsWallsTuning()) removeWalls();
        if (config.allowsWeightsTuning()) generateRelief();

        painter.setMaze(tunedMaze)
            .setCellColorF(v -> config.tuningColor(weights, v));
      }
      ...
      Platform.runLater(() -> {
        ...
        if (!canceled) {
          tuneWallsGroup.setDisable(false);
          tuneReliefGroup.setDisable(false);
  ...
  private void onSolve() {
    ...
    MazeSolver solver = solverChoiceBox.getValue().value();
    ...
    var wf = config.weightFunction(weights);
    painter
        .setWallColor(config.wallColor())
        .setCellColorF(v -> config.solverCellColor(source, destination, weights, resolutionMonitor, pathPred, wf, v));
    ...
    runOnWorker(() -> {
      try {
        ...
        Platform.runLater(() -> {
          solverMetadataList.clear();
          for (var e : r.metadata()) {
            solverMetadataList.add(e.getKey().name());
            solverMetadataList.add("-> " + e.getValue());
          }
        });
      } catch (CanceledAnimationException _) {
      } finally {
  ...
  private void onSelectVertex(MouseEvent event) {
    ...
    if (row < 0 || col < 0 || row >= generatedMaze.height() || col >= generatedMaze.width()) return;
  ...
  private void onOpen() {
    ...
    painter.setCellColorF(v -> config.tuningColor(weights, v));
  ...
  private void onSave() {
    ...
    MazeIO.write(maze, weights, source, destination, file);
  }
  ...
  private void paintSelection(int vertex, Color color) {
    ...
    int y = painter.cellOffset(vertex / generatedMaze.width());
  ...
  private void repaintMaze() {
    ...
    int approximateCellSide = (int) Math.min(canvasArea.getWidth() / maze.width(), canvasArea.getHeight() / maze.height());
    int wallThickness = config.wallThickness(approximateCellSide);
    int doubleThickness = 2 * wallThickness;

    // Garanti une taille min de 1 pour éviter une image noire
    int cellSide = Math.clamp(
        ((int) canvasArea.getWidth() - doubleThickness) / maze.width() - doubleThickness,
        1,
        ((int) canvasArea.getHeight() - doubleThickness) / maze.height() - doubleThickness
    );
    int width  = (cellSide + doubleThickness) * maze.width() + doubleThickness;
    int height = (cellSide + doubleThickness) * maze.height() + doubleThickness;
  ...
  public void init(Config config, MazeGenerator generator, List<ChoiceItem<MazeSolver>> solvers) {
    this.config = config;
    ...
    painter = new MazePainter(null, canvas.getGraphicsContext2D())
        .setWallColor(config.wallColor());
    tuner = new MazeTuner()
        .setReliefMinWeight(config.minWeight())
        .setReliefMaxSummitWeight(config.maxWeight());

    tuneWallsGroup.setVisible(config.allowsWallsTuning());
    tuneReliefGroup.setVisible(config.allowsWeightsTuning());
  }
  ...
  private void runOnWorker(Runnable task) {
    CompletableFuture.runAsync(task, worker)
        .exceptionally(e -> {
          e.printStackTrace();
          Platform.runLater(() -> {
            Alert alert = new Alert(Alert.AlertType.ERROR);
            alert.setTitle("Something happened™");
            alert.setHeaderText("Voir la console pour la stack trace complète.");
